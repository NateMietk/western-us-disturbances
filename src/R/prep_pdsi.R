...
start_date <- as.Date(paste("1984", "01", "01", sep = "-"))
...
# All of this only needs to be run once #
# nc <- nc_open("data/climate/pdsi/monthly_mean/pdsi_19792016.nc")
...
#   x - y
# }
# pdsi_anomalies <- overlay(x = pdsi_mm, y = pclimatology_mon, fun = fun)
# pdsi_anomalies = setZ(pdsi_anomalies, idx)
# names(pdsi_anomalies) <- paste(year(date_seq), month(date_seq), 
#                              day(date_seq), sep = "-")
                             
pdsi_anom_list <- list.files(file.path(prefix, "climate/pdsi","monthly_anomalies/"), 
                             pattern = ".tif", full.names = TRUE)
pdsi_anomalies <- stack(pdsi_anom_list)
pdsi_anomalies = setZ(pdsi_anomalies, idx)
names(pdsi_anomalies) <- paste(year(date_seq), month(date_seq), 
                                day(date_seq), sep = "-")

# Create summer only series
pdsi_june_anom <- subset(pdsi_anomalies,  grep(".6.1$", names(pdsi_anomalies)),
                         drop = TRUE) # subset based on year
pdsi_july_anom <- subset(pdsi_anomalies,  grep(".7.1$", names(pdsi_anomalies)),
                         drop = TRUE) # subset based on year
pdsi_august_anom <- subset(pdsi_anomalies,  grep(".8.1$", names(pdsi_anomalies)),
                           drop = TRUE) # subset based on year
...
# 
# pdsi_std_anom <- pdsi_anomalies / pdsi_sd
